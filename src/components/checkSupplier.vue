<template>
    <div>
      ...
      <div class="check">
        <div class="fl">
         经销商：
          <Input v-model="value" size="small" placeholder="搜‘18’试试？" style="width: 130px" @on-enter="getData(value)"></Input>
          <Button type="primary" size="small" @click="getData(value)" :style="{marginLeft:'20px',width:'50px'}">查询</Button>
        </div>
        <Button type="primary" size="small" :style="{marginLeft:'5px',width:'50px'}" @click="plus" :flag="flag" @change="plus">新增</Button>
      </div>
      ...
      <supplier v-show.sync="flag" :myname.sync = flag></supplier>
    </div>
</template>

<script>
  import supplier from './supplier/baseInfo'
  let testData = {
    "datas": [
      {
        name: '王小明',
